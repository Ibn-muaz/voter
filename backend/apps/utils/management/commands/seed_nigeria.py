...
class Command(BaseCommand):
    help = 'Seeds Nigerian states and LGAs'
    ...
    def handle(self, *args, **options):
        states_data = {
            ...
            'Osun': ['Atakunmosa East', 'Atakunmosa West', 'Aiyedaade', 'Aiyedire', 'Boluwaduro', 'Boripe', 'Ede North', 'Ede South', 'Ife Central', 'Ife East', 'Ife North', 'Ife South', 'Egbedore', 'Ejigbo', 'Ifelodun', 'Ila', 'Ilesa East', 'Ilesa West', 'Irepodun', 'Irewole', 'Isokan', 'Iwo', 'Obokun', 'Odo Otin', 'Ola Oluwa', 'Olorunda', 'Oriade', 'Orolu', 'Osogbo'],
        ...
        for state_name, lgas in states_data.items():
            # Use unique 3-letter codes
            code = state_name[:3].upper()
            state, created = NigerianState.objects.get_or_create(
                name=state_name,
                defaults={'code': code, 'capital': 'Unknown'}
            )
            if created:
            ...
            for lga_name in lgas:
                LocalGovernmentArea.objects.get_or_create(
                    name=lga_name,
                    state=state,
                    defaults={'code': f'{state.code}{lgas.index(lga_name):03}'}
                )
        
        self.stdout.write(self.style.SUCCESS('Successfully seeded states and LGAs'))
